...
uint8_t can0_tx_cnt,can1_tx_cnt,can2_tx_cnt;
...
void FlexCAN0_Init(void) {              /* General init. No MB IDs iniialized */
  ...
  for(i=0; i<64; i++){ 			//MPC574xP has 64 buffers
    CAN_0.MB[i].CS.B.CODE = 8;   /* Inactivate all message buffers */
  }
...
void FlexCAN1_Init(void) {              /* General init. No MB IDs iniialized */
	...
	for(i=0; i<64; i++)//MPC574xP has 64 buffers
	{
		CAN_1.MB[i].CS.B.CODE = 8;   /* Inactivate all message buffers */
	}
...
void FlexCAN2_Init(void) {              /* General init. No MB IDs iniialized */
	...
	for(i=0; i<64; i++)//MPC574xP has 64 buffers
	{
		CAN_2.MB[i].CS.B.CODE = 8;   /* Inactivate all message buffers */
	}
...
void CAN0_TransmitMsg(CAN_Packet m_CAN_Packet)
{
	...
	if(8 != can0_tx_cnt)
	{
		...
		for (i=0; i<m_CAN_Packet.length; i++)
		{
			CAN_0.MB[can0_tx_cnt].DATA.B[i] = m_CAN_Packet.data[i];      /* Data to be transmitted */
		}
		CAN_0.MB[can0_tx_cnt].CS.B.DLC = m_CAN_Packet.length; /*#bytes to transmit w/o null*/
		CAN_0.MB[can0_tx_cnt].CS.B.SRR = 1;     /* Tx frame (not req'd for standard frame)*/
		CAN_0.MB[can0_tx_cnt].CS.B.CODE =0xC;   /* Activate msg. buf. to transmit data frame */
	}
	can0_tx_cnt = (1 + can0_tx_cnt) % 64;
}
...
void CAN1_TransmitMsg(CAN_Packet m_CAN_Packet)
{
	...
	if(8 != can1_tx_cnt)
	{
		...
		for (i=0; i<m_CAN_Packet.length; i++)
		{
			CAN_1.MB[can1_tx_cnt].DATA.B[i] = m_CAN_Packet.data[i];      /* Data to be transmitted */
		}
		CAN_1.MB[can1_tx_cnt].CS.B.DLC = m_CAN_Packet.length; /*#bytes to transmit w/o null*/
		CAN_1.MB[can1_tx_cnt].CS.B.SRR = 1;     /* Tx frame (not req'd for standard frame)*/
		CAN_1.MB[can1_tx_cnt].CS.B.CODE =0xC;   /* Activate msg. buf. to transmit data frame */
	}
	can1_tx_cnt = (1 + can1_tx_cnt) % 64;
}
...
void CAN2_TransmitMsg(CAN_Packet m_CAN_Packet)
{
	...
	if(8 != can2_tx_cnt)
	{
		while(CAN_2.MB[can2_tx_cnt].CS.B.CODE != 0x8 ){}
		CAN_2.MB[can2_tx_cnt].CS.B.IDE = 0;       /* Use standard ID length */
		CAN_2.MB[can2_tx_cnt].ID.B.ID_STD = m_CAN_Packet.id;/* Transmit ID is 0x555 */
		CAN_2.MB[can2_tx_cnt].CS.B.RTR = 0;       /* Data frame, not remote Tx request frame */
		for (i=0; i<m_CAN_Packet.length; i++)
		{
			CAN_2.MB[can2_tx_cnt].DATA.B[i] = m_CAN_Packet.data[i];      /* Data to be transmitted */
		}
		CAN_2.MB[can2_tx_cnt].CS.B.DLC = m_CAN_Packet.length; /*#bytes to transmit w/o null*/
		CAN_2.MB[can2_tx_cnt].CS.B.SRR = 1;     /* Tx frame (not req'd for standard frame)*/
		CAN_2.MB[can2_tx_cnt].CS.B.CODE =0xC;   /* Activate msg. buf. to transmit data frame */
	}
	can2_tx_cnt = (1 + can2_tx_cnt) % 64;
}
